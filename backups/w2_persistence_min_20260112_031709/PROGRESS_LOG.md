...
## 20260111_xxxxxx - S2_netcore_scalable
- 状态：完成。实现了权威校验 + 限频 + AOI 网格 + 增量(delta)同步 + bot 压测脚本。
- 服务器改造：
    - AOI：九宫格 cell 广播，减少无关流量。
    - Delta Sync：仅广播变化玩家，降低带宽。
    - 权威校验：速度、频率、边界检查。
- 客户端改造：支持 Snapshot 和 Delta 混合更新。
- 压测：新增 ws_bot_swarm.js，20 bots 持续 10秒跑测，无报错。
- 验证：ws_dual_client_sim.js 仍通过。
