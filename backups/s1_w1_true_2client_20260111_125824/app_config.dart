...
class AppConfig {
  ...
  static Future<bool> discoverAndSetBaseUrl() async {
    ...
    final currentUrl = await getHttpBaseUrl();
    if (!currentUrl.contains("localhost")) {
      developer.log(
        'Base URL is already set to a non-local address, skipping discovery.',
    ...
    developer.log(
      'Testing candidate URL: $candidateUrl',
      name: 'AppConfig.Discovery',
    );

    try {
      final healthUri = Uri.parse(
        candidateUrl.endsWith('/')
            ? '${candidateUrl}health'
            : '$candidateUrl/health',
      );
      ...
      if (response.statusCode == 200) {
        developer.log(
          'Discovery successful! Setting Base URL to: $candidateUrl',
          name: 'AppConfig.Discovery',
        );
      ...
      developer.log(
        'Discovery failed for $candidateUrl: $e',
        name: 'AppConfig.Discovery',
      );
    }
    return false;
